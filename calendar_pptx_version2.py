...
# === CONFIGURATION ===
num_months = 9  # Change to any number 3–12
start_month = datetime(2025, 6, 1)
legend_top_offset_inch = 0.4  # Distance from bottom for legend

# Dynamic column count based on number of months
...
max_width = (slide_width - 1.2) / n_cols
max_height = (slide_height - 2.0) / n_rows

cell_width = Inches(max_width / 7)
cell_height = Inches(max_height / 8)

base_font = min(cell_width.inches, cell_height.inches) * 20
title_font = int(base_font * 1.8)
month_font = int(base_font * 1.4)
day_font = int(base_font * 1.1)
legend_font = int(base_font * 0.8)
...
x_margin = Inches(0.15)
y_margin = Inches(0.25)
...
# === SAVE ===
prs.save("calendar_dynamic_layout.pptx")